...
;	__CONFIG	_CONFIG1  & _CCP1_RB0 & _DEBUG_OFF & _CPD_OFF & _LVP_OFF & _BODEN_OFF & _MCLR_OFF & _PWRTE_ON & _WDT_OFF & _INTRC_IO
;*	ERRORLEVEL -302
...
despliega_u	BCF		STATUS,RP0
			MOVF		UNI,0					; mueve el contenido de UNI al registro W
												; RB3,RB2,RB1,RA6 	UNIDADES
												; RB7,RB6,RB5,RB4	DECENAS
			MOVWF		PORTB
			BSF		PORTB,4				; poniendo en 1 en el nibble mas alto, apaga el display de las DECENAS
			BSF		PORTB,5
			BSF		PORTB,6
			BSF		PORTB,7
			BTFSS		UNI,0					;salta si hay un '1' en el bit0 del registro UNI
			GOTO		set_cero					; si no, salta y pon un '0' en el bit6 del puerto A
			BSF		PORTA,6				; si si, pon un '1' en el bit6 del puerto A
			GOTO		termina_u
set_cero		BCF		PORTA,6
termina_u	RETURN	
;---------------------------------------------------------------------------------------------								
...
un_segundo			MOVLW		H'04'					;mueve 4 al registro contador	
					...
					MOVLW		H'FA'					;mueve 250 al registro contador	
...
;#################################################################################################
guardar_reg_imp		MOVWF 	W_TEMP 		;Copy W to TEMP register								;codigo propuesto por hoja de especificaciones
...
recuperar_reg_imp				MOVF 		PCLATH_TEMP,W 		;Restore PCLATH						;codigo propuesto por hoja de especificaciones
