...
INICIA_SP		CLRF		PORTB_com
				...
				;--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------				;----------------------------------------------------------------------------------------------				
OTRA_VEZ		MOVLW		H'FF'						;(1)								UTILIZADO PARA EL TIEMPO DE CONFIGURACION	
				MOVWF		CUENTASP1				;(1)								UTILIZADO PARA EL TIEMPO DE CONFIGURACION	
				MOVLW		H'FF'						;(1)								UTILIZADO PARA EL TIEMPO DE CONFIGURACION	
				MOVWF		CUENTASP2				;(1)								UTILIZADO PARA EL TIEMPO DE CONFIGURACION	
				MOVLW		H'0C'						;(1)	;carga un  decimal				UTILIZADO PARA EL TIEMPO DE CONFIGURACION	
				MOVWF		CUENTASP3				;(1)		255x255x15x1 micro sec = 
				;--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
REVISA_U		BTFSS		PORTB,7				;(1)									REVISA BOTON UNIDADES
				GOTO		REVISA_D				;(2)									REVISA BOTON UNIDADES
				;----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				CALL		t250_ms					; REBOTE
				...
				GOTO		OTRA_VEZ
				;-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
REVISA_D		BTFSS		PORTA,5				;(1)									REVISA BOTON DECENAS
				GOTO		QUITA_TIEMPO			;(2)			; 9 ciclos de instruccion, 	REVISA BOTON DECENAS
				;---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				CALL		t250_ms					; REBOTE
...
ACCION_out		BANCO0
				...
				MOVWF		CUENTASP1	;
				;------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
PARPADEA		CALL		desplegar_u_bcd ;despliega UNI
...
t250_micros		BANCO0					;selecciona banco 0
				MOVLW		H'52'			; (1) ciclo de instruccon
				MOVWF		T_ADQ			; (1)
...
si_es_cero_uni		MOVLW		H'09'
					MOVWF		UNI
					CALL		desplegar_u_bcd			;CALL		desplegar_u_bcd
					GOTO		si_sigue_opr
					;--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------																		
...
configura_puertos	BANCO1					;;Puerto A como salida		0 - salida; 	1 - entrada
				MOVLW		B'10100000'		; RA7	RA6		RA5		RA4		RA3		RA2		RA1		RA0
				MOVWF		TRISA			;    1		  0	          1		  0		  0		   0		   0		   0	
				;--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------										
				MOVLW		B'10000001'		; RB7	RB6		RB5		RB4		RB3		RB2		RB1		RB0
				MOVWF		TRISB			;    1		  0	          0		  0		  0 		   0		   0		   1		   
				;--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------						
				RETURN
...
desplegar_d_bcd		BANCO0
					...
					MOVWF		DECE_com			; mueve W al registro DECE_com, que es un registro comodin
					;--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
					RLF		DECE_com,1
					RLF		DECE_com,1
					RLF		DECE_com,1
					RLF		DECE_com,1			; 1- lo deja en el mismo registro.
					;-----------------------------------------------------------------------------------------------------			
					MOVLW		B'00001111'				; mueve la literal al registro W
					ANDWF		PORTB,0				; (PORTB AND W) ---> resultado en W
					IORWF		DECE_com,1			; (W OR DECE_com) ---> resultado en DECE_com
					MOVF		DECE_com,0	
					MOVWF		PORTB					; MUEVE EL REGISTRO YA enmascarado al puerto B
					;--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
					BTFSS		DECE_com,7
					GOTO		APAGA
					GOTO		ENCIENDE
					;--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
APAGA				BCF		PORTA,0
					GOTO		fin_desplegar_d_bcd
ENCIENDE			BSF		PORTA,0
					;--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
fin_desplegar_d_bcd	RETURN
					;----------------------------------------;------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
;##########################################################################################################################
...
BCD_BINARIO	MOVF		DECE,0				; mueve el contenido de DECE al registro W
				...
				MOVWF		TEMPERATURA_SP	; mueve W al registro TEMPERATURA_SP
